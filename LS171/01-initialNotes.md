...
- detailed example handshake steps: [^34]
  ...
  - **server** receives and responds with `ServerHello` : sets protocol version, cipher suite; sends certificate w/ public key (see also [Certificate](#certificate)); sends `ServerHelloDone`
  ...
  - **server** decrypts 'pre-master secret', uses agreed cipher suite to generate symmetric key from 'pre-master secret' (will be the same as client's) and sends `ChangeCipherSpec` and `Finished`
